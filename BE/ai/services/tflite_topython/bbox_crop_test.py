...
IMAGE_PATH = "C:\\Users\\SSAFY\\Desktop\\phone.png" # ★★★ 테스트할 입력 이미지 경로로 수정 ★★★
...
try:
    ...
    print("[WARNING] 'apple' class not found in CLASS_NAMES. Object detection for apple will not work.")
...
def postprocess_output_coco(
    ...
    for index in selected_indices:
        ...
        final_boxes_orig_coords.append([int(x1_orig), int(y1_orig), int(x2_orig), int(y2_orig)])
...
def detect_apples_and_get_bboxes(image_np):
    ...
    print(f"[INFO] Detected {len(apple_detection_results)} apple(s) during detection phase.")
...
def crop_and_save_bboxes_to_folder(image_np, bboxes_info, output_dir):
    ...
    for i, bbox_info in enumerate(bboxes_info):
        ...
        if x2 <= x1 or y2 <= y1:
            print(f"[WARNING] Invalid bounding box coordinates for cropping: {bbox_info['bbox']}. Skipping.")
            continue
        ...
        if cropped_image_np.size == 0:
            print(f"[WARNING] Cropped image for box {bbox_info['bbox']} resulted in empty array. Skipping.")
            continue
...
if __name__ == '__main__':
    ...
    if apple_detection_results:
        ...
        saved_paths = crop_and_save_bboxes_to_folder(original_image_test, apple_detection_results, TEST_OUTPUT_DIR)
